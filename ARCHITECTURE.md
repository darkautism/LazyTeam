...
- **Reviewer agents (protocol 6)** claim pinned review leases for tasks in `review`, verify the exact candidate independently from the Host broker, and finish through the slot-scoped `submit_review` MCP tool. The review slot owns its MCP endpoint, invalid-submit budget, and verdict lifecycle; the existing `finish_review` server path remains the only state-transition authority. Reviewer agents never modify source, commit, push, or merge.
