...
- **Reviewer agents (protocol 6)** claim pinned review leases for tasks in `review`, verify the exact candidate independently from the Host broker, and finish through the slot-scoped `submit_review` MCP tool. The review slot owns its MCP endpoint, invalid-submit budget, and verdict lifecycle; the existing `finish_review` server path remains the only state-transition authority, and the MCP tool itself does not mutate task/review state. Reused Pi reviewer sessions explicitly reactivate `submit_review` so the verdict tool stays active for the returning session. If the substantive review settles without a terminal verdict, LazyTeam issues exactly one same-session final `submit_review` nudge before treating the lease as a runtime failure. Reviewer agents never modify source, commit, push, or merge.
