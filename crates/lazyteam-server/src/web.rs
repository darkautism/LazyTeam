...
:root{color-scheme:dark;background:#0b0d10;color:#e7eaf0;font-family:ui-sans-serif,system-ui,sans-serif}body{margin:0}header{padding:24px 28px;border-bottom:1px solid #292e37;display:flex;justify-content:space-between;align-items:center}main{padding:24px;display:grid;gap:20px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(290px,1fr));gap:16px}.panel{background:#12161c;border:1px solid #292e37;border-radius:12px;padding:16px}h1,h2{margin:0 0 12px}small,.muted{color:#9098a6}.item{padding:10px 0;border-top:1px solid #252a32}.item:first-child{border-top:0}.tag{display:inline-block;padding:2px 7px;margin:2px;border:1px solid #3c4654;border-radius:10px;font-size:12px}.state{font-weight:700}button{background:#2b65d9;color:white;border:0;border-radius:7px;padding:6px 10px;cursor:pointer}button.secondary{background:#414854}.row{display:flex;gap:8px;justify-content:space-between;align-items:center;flex-wrap:wrap}.error{color:#ff8c8c}</style></head>
<body><header><div><h1>LazyTeam</h1><small>Multi-project AI worker control plane</small></div><button onclick="refreshAll()">Refresh</button></header>
...
async function json(url,opt){const r=await fetch(url,opt);if(!r.ok)throw new Error(await r.text()||r.statusText);if(r.status===204)return null;return r.json()}
