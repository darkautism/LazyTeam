...
<main><section class="grid"><div class="panel"><h2>Projects</h2><div id="projects">Loading…</div></div><div class="panel"><h2>Workers</h2><div id="workers">Loading…</div></div></section><section class="panel join"><h2>Worker join</h2><div class="row"><div><div>Generate a short-lived bootstrap code for a remote worker.</div><small class="muted">The code contains the public LazyTeam endpoint and expires after 10 minutes.</small></div><div><button onclick="generateJoinCode()">Generate join code</button> <button class="secondary" onclick="copyJoinCode()">Copy code</button></div></div><input id="worker-join-code" type="text" readonly placeholder="Generate a join code first"><small id="join-status" class="muted">No active join code</small><pre id="join-command" class="muted">Worker command will appear here.</pre></section><section class="panel"><h2>Tasks</h2><div id="tasks">Loading…</div></section></main>
<script>
const esc=s=>String(s??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]));
const TOKEN_KEY='lazyteam.adminToken';
const tokenInput=document.querySelector('#admin-token');
const authStatus=document.querySelector('#auth-status');
const joinInput=document.querySelector('#worker-join-code');
...
async function generateJoinCode(){try{const data=await json('/api/worker-join',{method:'POST'});joinInput.value=data.join_code;joinStatus.textContent=`Server ${data.server} · expires ${new Date(data.expires_at).toLocaleString()}`;joinStatus.className='ok';joinCommand.textContent=`cargo run -p lazyteam-worker -- --join-code '${data.join_code}' --name worker-01 --project '*' --tag rust=true --slots 1`}catch(e){joinStatus.textContent=e.message;joinStatus.className='error';joinInput.value='';joinCommand.textContent='Worker command will appear here.'}}
async function copyJoinCode(){if(!joinInput.value)return;try{await navigator.clipboard.writeText(joinInput.value);joinStatus.textContent='Join code copied';joinStatus.className='ok'}catch(e){joinStatus.textContent='Copy failed: '+e.message;joinStatus.className='error'}}
...
mod tests {
    ...
    fn admin_token_is_session_scoped_and_sent_as_bearer() {
        ...
        assert!(INDEX.contains("--join-code"));
        assert!(INDEX.contains("Generate join code"));
    }
}
